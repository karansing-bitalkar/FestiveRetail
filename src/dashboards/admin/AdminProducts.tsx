...
export default function AdminProducts() {
  ...
  const filtered = products.filter(p => !search || p.name.toLowerCase().includes(search.toLowerCase()) || p.festival.toLowerCase().includes(search.toLowerCase()));
  ...
  const handleSave = () => {
    ...
    if (editProd) { updateProduct({ ...editProd, ...form } as Product); toast.success('Product updated!'); }
    else { addProduct({ id: `admin${Date.now()}`, vendorId: 'admin', rating: 4.5, reviews: 0, isCombo: false, tags: [], ...form } as Product); toast.success('Product added!'); }
    setModal(false);
  };
  ...
  return (
    <div className="flex flex-col gap-6">
                ...
                <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }} className="hover:bg-orange-50/30 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-semibold text-gray-900 text-sm max-w-[180px] truncate">{p.name}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-4"><span className={`text-xs font-bold px-2 py-1 rounded-full ${p.stock < 10 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-700'}`}>{p.stock}</span></td>
      ...
      <ConfirmModal isOpen={!!deleteId} title="Delete Product" message="Are you sure you want to delete this product? This cannot be undone."
