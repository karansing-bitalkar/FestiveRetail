...
import { Mail, Phone, MapPin, Clock, Send, Check, Sparkles, MessageCircle, Headphones } from 'lucide-react';
    ...
    <div>
      ...
      <section className="py-20 fest-gradient relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center text-white">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/20 rounded-full text-sm font-medium mb-5"><Sparkles size={14} /> Get In Touch</div>
      ...
      <section className="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: Phone, label: 'Call Us', value: '1800 123 4567', sub: 'Toll Free · Mon–Sun', color: 'bg-orange-50 text-orange-500' },
            { icon: Mail, label: 'Email Us', value: 'support@festiveretail.com', sub: 'Reply within 4 hours', color: 'bg-blue-50 text-blue-500' },
            { icon: MessageCircle, label: 'WhatsApp', value: '+91 98765 43210', sub: 'Quick responses', color: 'bg-green-50 text-green-500' },
            { icon: Headphones, label: 'Live Chat', value: 'Available 24/7', sub: 'Instant support', color: 'bg-purple-50 text-purple-500' },
          ].map((c, i) => (
            <motion.div key={c.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              ...
              <div className={`w-12 h-12 ${c.color} rounded-xl flex items-center justify-center mx-auto mb-3`}><c.icon size={20} /></div>
              ...
              {OFFICES.map(o => (
                <div key={o.city} className="bg-orange-50 border border-orange-100 rounded-2xl p-5">
                  ...
                  <div className="flex flex-col gap-2 text-sm">
                    <div className="flex items-start gap-2 text-gray-600"><MapPin size={14} className="text-orange-400 mt-0.5 flex-shrink-0" />{o.address}</div>
                    <div className="flex items-center gap-2 text-gray-600"><Phone size={14} className="text-orange-400 flex-shrink-0" />{o.phone}</div>
                    <div className="flex items-center gap-2 text-gray-600"><Mail size={14} className="text-orange-400 flex-shrink-0" />{o.email}</div>
                  </div>
                </div>
              ))}
            </div>
            <div className="bg-gray-900 text-white rounded-2xl p-6">
              <h4 className="font-bold mb-3 flex items-center gap-2"><Clock size={15} className="text-orange-400" /> Support Hours</h4>
              <div className="flex flex-col gap-2 text-sm">
